...
$query = "SELECT MONTH(FROM_UNIXTIME(attack_date)) AS month, YEAR(FROM_UNIXTIME(attack_date)) AS year, SUM(attack_metal) AS metal, SUM(attack_cristal) AS cristal, SUM(attack_deut) AS deut, SUM(attack_pertes) as pertes FROM " . TABLE_ATTAQUES_ATTAQUES . " WHERE attack_user_id=" . $user_data['id'] . " AND MONTH(FROM_UNIXTIME(attack_date)) <> $mois GROUP BY month, year";
...
if ($nb_result != 0) {
    ...
    while (list($month, $year, $metal, $cristal, $deut, $pertes) = $db->sql_fetch_row($result)) {
        ...
        $query = "SELECT attack_coord, attack_date, attack_metal, attack_cristal, attack_deut, attack_pertes FROM " . TABLE_ATTAQUES_ATTAQUES . " WHERE attack_user_id=" . $user_data['id'] . " AND MONTH(FROM_UNIXTIME(attack_date))=" . $month . " AND YEAR(FROM_UNIXTIME(attack_date))=" . $year . "";
        ...
        $query = "SELECT SUM(recy_metal) as metal_recy, SUM(recy_cristal) as cristal_recy FROM " . TABLE_ATTAQUES_RECYCLAGES . " WHERE recy_user_id=" . $user_data['id'] . " AND MONTH(FROM_UNIXTIME(recy_date))=" . $month . " AND YEAR(FROM_UNIXTIME(recy_date))=" . $year . " GROUP BY recy_user_id";
        ...
        $query = "INSERT INTO " . TABLE_ATTAQUES_ARCHIVES . " (`archives_user_id` , `archives_nb_attaques` , `archives_date` , `archives_metal` , `archives_cristal` , `archives_deut` , `archives_pertes`, `archives_recy_metal`, `archives_recy_cristal` )
                VALUES (" . $user_data['id'] . ", $nb_ancattack, $timestamp, $metal, $cristal, $deut , $pertes, $metal_recy, $cristal_recy)";
        $db->sql_query($query);

        //On supprime les attaques qui viennent d'être sauvegardées
        $query = "DELETE FROM " . TABLE_ATTAQUES_ATTAQUES . " WHERE attack_user_id= " . $user_data['id'] . " AND MONTH(FROM_UNIXTIME(attack_date))=$month AND YEAR(FROM_UNIXTIME(attack_date))=$year";
        $db->sql_query($query);

        //On supprime les recyclages qui viennent d'être sauvegardés
        $query = "DELETE FROM " . TABLE_ATTAQUES_RECYCLAGES . " WHERE recy_user_id=" . $user_data['id'] . " AND MONTH(FROM_UNIXTIME(recy_date))=$month AND YEAR(FROM_UNIXTIME(recy_date))=$year";
        $db->sql_query($query);
